...
| March 2026 | H1 operational definition changed from `exploitation_loop_detection(target_selections)` to `interrogative_collapse_rate` proxy | Original implementation passed signal types (3 values: DECLARE/QUERY/RESPOND) instead of resource target IDs (up to 400 grid cell values) to a Shannon entropy function designed for spatial target diversity. 3-way entropy ceiling makes the 0.3 threshold essentially unreachable — the instrument was broken, not the hypothesis. `interrogative_collapse_rate` directly measures the predicted failure mode: agents locked in non-interrogative patterns. | Improves theoretical alignment. H1 prediction unchanged — unconstrained agents should show higher exploitation behavior, now operationalized as sustained low query rates rather than target selection entropy. Hash updated. |
| March 2026 | `query_rate` field in `epoch_series.json` was always `None` for all confirmatory runs due to wrong key lookup; `_epoch_query_rates` was always 0.0, making collapse detection in epoch_series unreliable | `engine.py:_write_epoch_series` used `inq.get("query_emergence_rate")` and `protocols.py:Protocol2.compute_epoch_extras` used `inquiry.get("query_emergence_rate", 0.0)` — this key does not exist in the dict returned by `compute_inquiry_metrics()`. Correct key is `type_distribution["QUERY"]`. Discovered during post-run data validation of seeds 0 and 1 (fixed code runs). Bug caused: (1) `query_rate` always `None` in epoch_series → `exploitation_loop_rate` always `None`; (2) `_epoch_query_rates` always `[0.0, 0.0, ...]` → `collapse_detected` fired from epoch 9 in all seeds due to artificial all-zeros input. Fix: `engine.py` line 536 changed to `(inq.get("type_distribution") or {}).get("QUERY")`; `protocols.py` line 328-329 changed to extract from `type_distribution`. Buggy epoch_series files for seeds 0 and 1 deleted and both seeds re-run with corrected code. Seeds 2–9 run for the first time with corrected code. | `exploitation_loop_rate` metric now correctly populated. `collapse_detected` field in epoch_series now reflects actual query rate dynamics. `sustained_structure_score` and `gaming_flag` were unaffected (use `type_entropy`, `qrc`, and `ethical_cost`, all correctly computed). Seeds 0 and 1 re-run; all confirmatory data is based on fixed code. Hash updated. |
